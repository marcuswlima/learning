...
class Program{
        ...
        static void Main()
        {
			...
			#region //codigo
			int[] intVector = {9,8,7,6,4356456,5,4,3,2,1};
			string[] myArray = {"marcus","um","dois"};
			
			for (int i=0; i<=myArray.Length-1;i++){
				Console.Write("i->"+i+"\t");
				Console.Write("myArray[i]->"+myArray[i]+"\t");
				Console.Write("\n");
			}
			Console.Write("myArray[0]->"+myArray[0]+"\n");
			Console.Write("myArray[1]->"+myArray[1]+"\n");
			
			int[] numeros= new int[5];
			for (int i=0;i<=numeros.Length-1;i++){
				numeros[i]=IntRandom(1,1550);
			}

			PrintStrVector(arrei:myArray);
			PrintIntVector(titulo:"intVector",arrei:intVector);
			PrintIntVector(arrei:numeros,titulo:"Números");
			...
			PrintIntMatriz(titulo:"Matriz de Inteiros",matriz:intMatrix);
			
			string[,] strMatrix = {
				{"In","this","example","we"},
				{"can","access","each","element"},
				{"using","indices","If","we"}, 
				{"assign","the","index","for"}, 
				{"Maria","Socorro","Soares","do"},
				{"rows","to","row","and"}, 
				{"the","index","for","columns"},
				{"Marcus","William","Santos","Lima"}
			};
			PrintStrMatriz(matriz:strMatrix,titulo:"Teste de Impressão");
